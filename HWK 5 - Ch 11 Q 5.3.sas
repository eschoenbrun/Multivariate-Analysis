...
PROC LOGISTIC data= Churn_new plots=all ;
CLASS V_CSC (ref='0') Int_l_Plan (ref='no') VMail_Plan (ref='no')/ param=ref;
MODEL churn = Account_Length Int_l_Plan VMail_Plan Day_Mins Eve_Mins Night_Mins Intl_Mins V_CSC;
run ;


title "Regression of churn on International Plan indicator, Voice Plan indicator, V_CSC2, Account Length, Day Minutes, Evening Minutes, Night Minutes and International Minutes
";
data Churn_new2;
set Churn_new;
...
if CustServ_Calls < 2 then V_CSC2=0;
else if CustServ_Calls < 4 then V_CSC2=1;
else V_CSC2=2;
